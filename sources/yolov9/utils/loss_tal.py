...
class ComputeLoss:
    ...
    def __init__(self, model, use_dfl=True):
        ...
        if os.getenv("YOLO_FOCAL_CLS") == "1":
            gamma = float(os.getenv("YOLO_FOCAL_GAMMA", g or 2.0))
            alpha = float(os.getenv("YOLO_FOCAL_ALPHA", 0.25))
            BCEcls = FocalLoss(BCEcls, gamma, alpha)
        elif g > 0:

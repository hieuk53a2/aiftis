...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.use_focal_cls = os.getenv("YOLO_FOCAL_CLS") == "1"
        self.focal_gamma = float(os.getenv("YOLO_FOCAL_GAMMA", "2.0"))
        self.focal_alpha = float(os.getenv("YOLO_FOCAL_ALPHA", "0.25"))
    ...
    def cls_loss_tensor(self, pred_scores: torch.Tensor, target_scores: torch.Tensor, dtype: torch.dtype) -> torch.Tensor:
        """Return classification BCE tensor, optionally modulated by focal loss."""
        target = target_scores.to(dtype)
        cls_loss = self.bce(pred_scores, target)
        if self.use_focal_cls:
            ...
            cls_loss *= alpha_factor * (1.0 - p_t).pow(self.focal_gamma)
        if self.class_weights is not None:
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        bce_loss = self.cls_loss_tensor(pred_scores, target_scores, dtype)  # (bs, num_anchors, nc)
        loss[1] = bce_loss.sum() / target_scores_sum  # BCE or focal BCE
...
class v8OBBLoss(v8DetectionLoss):
    ...
    def loss(self, preds: dict[str, torch.Tensor], batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        bce_loss = self.cls_loss_tensor(pred_scores, target_scores, dtype)  # BCE or focal BCE
